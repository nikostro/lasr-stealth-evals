...
eval-test:
	$(INSPECT_EVAL_COMMON) --max-connections 400 --time-limit 300 --fail-on-error $(FAIL_ON_ERROR) --epochs 5 --max-retries 3 -T monitor_model=openrouter/openai/gpt-4.1 -T turns=5 -T use_hint=False -T use_justification=False -T is_cot_monitored=False -T generation_strategy=3 -T benign_task=True
...
upload:
	uv run src/lasr_stealth_evals/collusion/wandb_logger.py --eval-dir $(EVAL_DIR) --run-name $(RUN_NAME)
