...
eval-test:
	$(INSPECT_EVAL_COMMON) --max-connections 400 --fail-on-error $(FAIL_ON_ERROR) --epochs 1 --max-retries 3 -T monitor_model=openrouter/google/gemini-2.5-flash-preview -T turns=5 -T use_hint=False -T use_justification=True -T is_cot_monitored=False -T generation_strategy=3
