...
def main(args: argparse.Namespace):
    with wandb.init(
        ...
        entity="fdmi",
        config=vars(args),
        group=f"strategy_{args.strategy.name}",
        name=f"{args.model}_run_{int(time.time())}",  # or include task_id if available
        reinit=True,
    ) as run:
        # Log input artifacts
        ...
        task_obj = task_fn(
            ...
            turns=args.turns,
            use_hint=args.hint,
            use_justification=DEFAULT_USE_JUSTIFICATION,
            is_cot_monitored=args.monitor_cot,
            monitor_model=args.monitor_model,
        )

        # Run it via Inspect AI engine
        log = task_obj.eval(
            model=args.model,
            epochs=args.epochs,
            limit=args.limit,
        ...
        # Extract metadata and metrics (assuming 1 run per script)
        run_metadata = log[0].metadata
        task_id = run_metadata.get("task_id", "unknown")
        strategy = args.strategy.name

        # Assuming your scorer includes accuracy and stderr
        scores = log[0].scores
        accuracy_score = scores.get("accuracy", 0)
        stderr_score = scores.get("stderr", 0)
        ...
        wandb.log({
            ...
            "stderr": stderr_score,
            "monitor_model": args.monitor_model,
            "agent_cot": args.agent_cot,
            "monitor_cot": args.monitor_cot,
            "turns": args.turns,
            "hint_used": args.hint,
        })

        # Log as wandb Table for structured team comparison
        table = wandb.Table(columns=[
            "task_id", "strategy", "accuracy", "stderr",
            "model", "monitor_model", "turns", "hint_used"
        ])
        table.add_data(
            task_id, strategy, accuracy_score, stderr_score,
            args.model, args.monitor_model, args.turns, args.hint
        )
        wandb.log({"task_summary": table})

        # Log transcript or log artifact
        artifact = wandb.Artifact(f"log_{task_id}", type="log")
        artifact.add_file(str(log[0].location))
        run.log_artifact(artifact, name=f"log_{task_id}")
